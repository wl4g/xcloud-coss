 ...
 */
package com.wl4g.devops.coss.client.config;
...
/**
 * Client configurations for accessing to COSS services.
 * 
...
public class ClientCossConfiguration {
	...
	/**
	 * Gets the difference between customized epoch time and local time, in
	 * millisecond.
	 *
	 * @return tick Offset
	 */
	...
	/**
	 * Sets the custom base time. OSS's token validation logic depends on the
	 * time. It requires that there's no more than 15 min time difference
	 * between client and OSS server. This API calculates the difference between
	 * local time to epoch time. Later one other APIs use this difference to
	 * offset the local time before sending request to OSS.
	 *
	 * @param epochTicks
